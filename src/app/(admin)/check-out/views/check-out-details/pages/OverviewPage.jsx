...
const stats = [
  { label: 'Inspection', value: 'Completed', icon: 'solar:calendar-date-bold-duotone', color: '#6747ff', bg: '#eee7ff' },
  { label: 'Repair & Damage', value: '2 Items', icon: 'solar:gallery-check-bold-duotone', color: '#47c878', bg: '#e9f8ef' },
  { label: 'Utility Charges', value: 'OMR 1962', icon: 'solar:user-plus-bold-duotone', color: '#ff8d3c', bg: '#fff0e8' },
  { label: 'Outstanding', value: 'OMR 1234', icon: 'solar:chart-2-bold-duotone', color: '#36c8cf', bg: '#e8fbfb' },
];
...
const timelineItems = [
  ['Inspection Completed', 'Inspection has been completed by Ramesh Kumar', '29 May 2026, 02:35 PM', '#d8f6dc'],
  ['Utility Readings Captured', 'Utility Readings were recorded', '29 May 2026, 02:35 PM', '#f5f3d8'],
  ['Repair & Damage Added', '2 repair & damage items added', '29 May 2026, 02:35 PM', '#dce9fb'],
  ['Check-Out Requested', 'Check-Out request was raised by Bilal Ahmed', '29 May 2026, 02:35 PM', '#ead9f8'],
];
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      <div style={{ display: 'grid', gap: 24, gridTemplateColumns: 'minmax(650px, 1.7fr) minmax(390px, 1fr)' }}>
            ...
            <div style={{ padding: '52px 28px 34px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', position: 'relative' }}>
                ...
                {progressSteps.map(([label, value, color, icon], index) => (
                  <div key={label} className="text-center" style={{ position: 'relative', zIndex: 1 }}>
                    <span
                      className="d-inline-flex align-items-center justify-content-center"
                      style={{
                        background: ['#dff9ff', '#eee4ff', '#ffd9d9', '#d7ffe0', '#ffd9ff', '#ffe9cc', '#ffe9cc'][index],
                        borderRadius: '50%',
                        height: 60,
                        marginBottom: 12,
                        width: 60,
              ...
              <div style={{ alignItems: 'center', display: 'grid', gap: 22, gridTemplateColumns: '150px 1fr 140px', marginTop: 54 }}>
                ...
                <span style={{ background: '#d8d8d8', borderRadius: 999, height: 10, overflow: 'hidden' }}>
                  <span style={{ background: '#6f95d2', borderRadius: 999, display: 'block', height: '100%', width: '65%' }} />
                </span>
                <strong style={{ color: pageText, fontSize: 16 }}>65% Completed</strong>
              </div>
            </div>
          </div>
          ...
          <div className="mb-4" style={cardStyle}>
            ...
            <div style={{ padding: '20px 34px 26px' }}>
              {timelineItems.map(([title, description, date, color], index) => (
                <div key={title} style={{ display: 'grid', gap: 20, gridTemplateColumns: '30px 1fr 210px', minHeight: 75, position: 'relative' }}>
                  {index < timelineItems.length - 1 && (
                    <span style={{ background: '#e1e1e1', height: 75, left: 14, position: 'absolute', top: 25, width: 1 }} />
                  )}
                  <span style={{ background: color, borderRadius: 5, height: 30, position: 'relative', width: 30, zIndex: 1 }} />
                  <div>
                    <p className="mb-2" style={{ color: pageText, fontSize: 15 }}>
                      {title}
                    </p>
        ...
        <div>
          <div className="mb-4" style={cardStyle}>
            <h5 style={titleStyle}>Inspection Summary</h5>
          ...
          <div className="mb-4" style={cardStyle}>
            <h5 style={titleStyle}>Financial Summary</h5>
          ...
          <div style={cardStyle}>
            <h5 style={titleStyle}>Key Return</h5>
            <div style={{ padding: '22px 74px 28px' }}>
